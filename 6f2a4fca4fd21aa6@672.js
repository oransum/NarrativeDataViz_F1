...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([["f1_dr_cons_relation_final",new URL("./files/cdc1e7329ebb6a051dbbc82e16c51883729950a59cf4f62e29b5607a6a4677a8eafe2d9f64770791d3898ebbb31641f734bfd52bcaf26681839bda30b8e0c2ff",import.meta.url)],["f1_constructorpoint_full.csv",new URL("./files/6b5b7cc9f310b48f39e2cdaf02ceb5c691d5297182847506fec34b00fefdca3ea4330e46a91c87837125fc3be8adfa14afef5e7db99e3a4f45c53077cb9eb7d8",import.meta.url)],["f1_driverwin.csv",new URL("./files/a7341994b9ea6dbdc7d3d6db1260e982d2969c3f3fb919185d9b7b450965cae023da3ce3016ffd9740f03a31371b9a35dddf8ba5ab4ec2d1efa431d3e0ffb4ef",import.meta.url)]]);
...
`
  ...
  main.variable(observer("chart_1")).define("chart_1", ["replay","d3","width","height_1","bars","axis","labels","ticker","keyframes","duration","x_1","invalidation"], async function*(replay,d3,width,height_1,bars,axis,labels,ticker,keyframes,duration,x_1,invalidation)
{
  replay;
...
- Upon the start of the race, Alain **Prost** is the leader and quickly followed by the rise of Ayrton **Senna** in *1987*. <br>

  - The rivalry between the two legends became F1's central focus during 1988, and continued until **Prost** retired at the end of 1993. 
...
- **Michael Schumacher** becomes the first German World Drivers' Champion in 1994 and it didn't stop him from gaining more Grand Prix wins to a legendary record, **91** wins. You may want to click the **Replay Race graph** button to see his rapid rise from *1994* and strong growth until *2006*.

  - He is the only driver in history to win **7 Formula One World Championships**(highest point in a season), five of which he won consecutively(2000 - 2004), with *Ferrari*.
  - In the mean time, it was **Prost** and **Senna**, who had retried for an decade, hold the second and the third place.
`
)});
  main.variable(observer()).define(["md"], function(md){return(
md`
#### '00s - '10s: The Youngest Champion(s)
- Schumacher's championship streak ended on 25 September *2005*, when Fernando **Alonso** became the first Spanish F1 driver to win the World Championship and was the youngest one-time and two-time drivers' champion(2006) at the time of his successes with *Renault*.

  - Before Alonso topped his record with **31** wins, he was having a intense competition with other young drivers. In meantime, Lewis **Hamilton** and Sebestian **Vettel** were working their way up to the leader pack.

**Hamilton** was the youngest champion in *2008* with *McLaren* and **Vettel** broke the record in *2010* and won consecutive titles in *2010–2013* with *Red Bull Racing*.

- **Vettel** surpassed **Prost**'s wins record in *2018*. He departed from *Red Bull* and joined *Ferrari* in 2015.
- **Hamilton** is a six-time Formula One World Champion, having won five with *Mercedes* which made him in both title and wins record the second-most of all time.
`
)});
...
- Mouse-over any line would enable you to visualize the constructor performance over the years.
`
)});
  main.variable(observer("chart_2")).define("chart_2", ["d3","width","height","xAxis_2","yAxis_2","data_const","line","hover"], function(d3,width,height,xAxis_2,yAxis_2,data_const,line,hover)
{
 ...
 // .attr("stroke", function(d) { return d.c; })/
  ...
  main.variable(observer()).define(["md"], function(md){return(
md`
- With the success of Hamilton in **Mercedes**, it is clearly that the German constructor took the lead from year *2015 to 2020*.
- Back to *2010 - 2013*, there were tight battles between the leader **Red Bull** and other teams such as **Ferrari** and **Mclaren**.
- In 2010s, **Ferrari** is not doing as good as the past decades.
`
)});
  main.variable(observer()).define(["md"], function(md){return(
md`## III. Success of Legends, Drivers and Constructors (2010 - 2020)`
)});
  main.variable(observer()).define(["md"], function(md){return(
...
- Driver of Season 2020:
  - The coloured curve indicates the current contract between a driver and a team, while the dimmed curve were their past contracts.

- Legends: 
  - Dimmed curve indicates a past relationship with a active constructor.
...
* *So how do you accomplish those?* Below, I describe how I created **\`\`district_DF\`\`** (step by step). You can apply similar steps to create **\`\`district_year_DF\`\`**.   

**${tex`\rightarrow`} If you want to figure out how to do this on your own, simply skip the bullet points**

>**1)** Filter the full dataframe (\`\`df\`\`) to only include focuscrimes.  
>**2)** The dataframe (\`\`df\`\`) only needs to include two columns, \`\`["PdDistrict","Category"]\`\`- others can be removed.    
>**3)** Create a **new** dataframe, \`\`district_DF\`\`, with one column - \`\`PdDistrict\`\`. Drop duplicates, so there is only one column and 10 rows.  
>**4)**  Create a \`\`for loop\`\` which goes through the list of focus crimes. For each focus crime you filter the dataframe to only include the current crimetype (of the \`\`for loop\`\`).  
> **5)** Now, aggregate the filtered dataframe - groupby PdDistrict and count the category column.  
> **6)** Rename the \`\`Category\`\` column with the name of the focuscrime you were just filtering on.   
> ** 7)** Use [\`\`pandas merge\`\`](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.merge.html) to merge the aggregated statistics for the focus crime to the dataframe you created before, \`\`district_DF\`\`.   
> ** 8)** Now you have all columns - except \`\`Total\`\` - I am sure you can manage adding it on your own 💪
